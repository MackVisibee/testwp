 ...
 * Description: Populate your site with random generated content. You can configure the post type, description, excerpt, tags, post meta, terms, images, publish date, status, parent, author, sticky, etc.
 * Text Domain: spp
 * Domain Path: /langs
 * Version: 4.2.3
 * Author: Iulia Cazan
...
define( 'SPP_PLUGIN_VERSION', 4.23 );
...
class SISANU_Popupate_Posts {
	...
	public static function load_assets() {
		...
		if ( file_exists( SPP_PLUGIN_DIR . 'build/style-index.css' ) ) {
			wp_enqueue_style(
				'spp-custom',
				SPP_PLUGIN_URL . 'build/style-index.css',
				[],
				filemtime( SPP_PLUGIN_DIR . 'build/style-index.css' ),
				false
			);
		}
	}
	 ...
	 *
	 * @param  integer $radius    The radius in km.
				...
				<div class="options-boxes">
							...
							<div id="spp_hint_gutenberg_template" class="spp_hint not-visible">
								...
								<p>
									...
									echo wp_kses_post( sprintf(
										// Translators: %1$s - pattern, %2$s - pattern, %3$s - pattern, %4$s - pattern, %5$s - pattern.
										__( 'For more accuracy in configuring the blocks, you can use in the template the following patterns: <ol><li>%1$s</li><li>%2$s</li><li>%3$s</li><li>%4$s</li><li>%5$s</li></ol>', 'spp' ),
										'<code>#[POST_ID]</code>',
										'<code>#[POST_TITLE]</code>',
										'<code>#[POST_EXCERPT]</code>',
										'<code>#[FEATURED_IMAGE_ID]</code>',
					...
					<div>
						...
						<div class="row-span one-two">
							...
							<em><?php esc_html_e( 'the user ID (leave empty or 0 for using the current user)', 'spp' ); ?></em>
						</div>
					...
					<div>
						...
						<div id="spp_hint_patterns" class="spp_hint not-visible">
							...
							<p>
								...
								echo sprintf( // phpcs:ignore
									// Translators: %1$s - letter pattern, %2$s - number pattern, %3$s - text pattern.
									esc_html__( 'Pattern examples: %1$s, %2$s, %3$s, %4$s, %5$s, %6$s, %7$s, %8$s, %9$s, %10$s, %11$s, %12$s, %13$s, %14$s, %15$s.', 'spp' ),
									...
									'<a onclick="sppApplyPattern(\'#[DATE]\');">' . esc_html__( 'date', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[TIME]\');">' . esc_html__( 'time', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[DATETIME]\');">' . esc_html__( 'date & time', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[LON]\');">' . esc_html__( 'longitude', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[LAT]\');">' . esc_html__( 'latitude', 'spp' ) . '</a>'
								);
								?>
							</p>
							<div class="row-span two-one">
						...
						<div class="row-span four-one">
							<?php esc_html_e( 'Click the info icon to see more about the import/export.', 'spp' ); ?>
				...
				if ( ! empty( $post_id ) ) {
					...
					if ( ! empty( $photos ) ) {
						...
						if ( ! empty( $photo ) ) {
							...
							do_action( 'spp_after_post_image_attached', (int) $post_id, (int) $photo, $photo_src, $description );
						}
					}
	...
	public static function plugin_admin_notices() {
		...
		if ( ! empty( $maybe_trans ) ) {
			...
			<div id="item-<?php echo esc_attr( $slug ); ?>" class="updated notice">
				<div class="icon">
					<a href="<?php echo esc_url( admin_url( 'tools.php?page=populate-posts-settings' ) ); ?>"><img src="<?php echo esc_url( SPP_PLUGIN_URL . 'assets/images/icon-128x128.gif' ); ?>"></a>
				</div>
				<div class="content">
					<div>
						<h3>
